    ...
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            private struct ContentStruct
            {
                ...
                // Atributo adicionado na customizacao pelo SUBORD
                private string m_formText;
                public string FormText
                {
                    get { return m_formText; }
                    set { m_formText = value; }
                }
            ...
            public static void SaveAsXml(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                foreach (IDockContent content in dockPanel.Contents)
                {
                    ...
                    // Modificacao do SUBORD para o SMC:
                    // Salvamos o caption do form para, ao carregar o XML, poder passar os parametros corretos 
                    // aos formularios que possuem parametros em seus construtores (FrmSimpleTable e FrmDataFields)
                    xmlOut.WriteAttributeString("FormText", content.ToString().Substring(content.ToString().IndexOf(", Text: ") + 8));
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn)
            {
                ...
                for (int i = 0; i < countOfContents; i++)
                {
                    ...
                    // Customizacao pelo SUBORD para o SMC: precisamos obter o Text do form
                    contents[i].FormText = xmlIn.GetAttribute("FormText");
            ...
            public static void LoadFromXml(DockPanel dockPanel, Stream stream, DeserializeDockContent deserializeContent, bool closeStream)
            {
                ...
                {
                    // Customizacao pelo SUBORD para o SMC
                    IDockContent content = deserializeContent(contents[i].PersistString, contents[i].FormText);
